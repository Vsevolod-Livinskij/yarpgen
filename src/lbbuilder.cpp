...
std::shared_ptr<IterDeclStmt> LBBuilder::genCoreIter (std::shared_ptr<Vector> v, uint64_t A, uint64_t level) {
...
void LBBuilder::add_me_some_iscls(std::shared_ptr<Vector> v_in) {
    ...
    uint64_t dicesize = 20;
    if (((*v_in)[0])->is_scalar())
        for(uint64_t i = 0; i < v_in->size(); ++i)
            if(rand_val_gen->get_rand_value<uint64_t>(0, dicesize) == 0)
...
LBBuilder::LBBuilder (std::shared_ptr<Context> ctx, std::vector<std::shared_ptr<Vector>> input)
         : Stmt(Node::NodeID::LBB) {
    this->context = ctx;
    this->tripcount = 0;
    this->parent = NULL;
...
LBBuilder::LBBuilder (LBBuilder *p, std::vector<std::shared_ptr<Vector>> input)
    ...
    assert(p != NULL);
    this->parent = p;
    this->context = std::make_shared<Context>(*(p->context->get_gen_policy()), p->context, Node::NodeID::LBB, true);
...
std::vector<std::shared_ptr<ScalarVariable>> LBBuilder::cherryPick(std::vector<std::shared_ptr<ScalarVariable>> &vars) {
    std::vector<std::shared_ptr<ScalarVariable>> ret;
    if (vars.size() == 0) return ret;

    uint64_t maxnum = 2;
    uint64_t cnt = rand_val_gen->get_rand_value<uint64_t>(1, std::min((uint64_t)vars.size(), maxnum));

    for (uint64_t i = 0; i < cnt; ++i) {
        uint64_t id = rand_val_gen->get_rand_value<uint64_t>(0, vars.size() - 1);
...
void LBBuilder::generate (std::vector<std::shared_ptr<Vector>> input) {
    ...
    if (this->context->get_loop_depth() < 4) {
        int cvid = rand_val_gen->get_rand_value<uint64_t>(0, input.size() - 1);
        core_vec = input[cvid];
    }
        ...
        int cvid = rand_val_gen->get_rand_value<uint64_t>(0, oinput.size() - 1);
    ...
    for (auto v : input) {
        if ((input.size() <= 20) &&
            ...
            (rand_val_gen->get_rand_value<uint64_t>(0, 3) == 0)) {

            uint64_t ovp = rand_val_gen->get_rand_value<uint64_t>(0, this->tripcount/5);
    ...
    while ((this->oscls.size() > 0) || (this->ovecs.size() > 0)) {
        ...
        std::vector<Probability<Parts>> decisions;
        decisions.push_back(Probability<Parts>(Parts::DO_TMP, ((this->ivecs.size() == 0) || (this->context->get_complexity() > 500000)) ? 0 : 20));
        decisions.push_back(Probability<Parts>(Parts::DEEPER, (this->ovecs.size() == 0) ? 0 : 30));
        decisions.push_back(Probability<Parts>(Parts::SCALAR, (this->oscls.size() == 0) ? 0 : 50));
        Parts decision = rand_val_gen->get_rand_id(decisions);
        ...
        if ((this->oscls.size() != 0) && (decision == Parts::SCALAR)) {
            std::vector<std::shared_ptr<ScalarVariable>> o_candidates = this->cherryPick(this->oscls);
