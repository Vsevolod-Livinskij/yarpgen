...
OPT=-O3
LDFLAGS=-L./ -std=c++14
LIBSOURCES=type.cpp variable.cpp expr.cpp stmt.cpp gen_policy.cpp sym_table.cpp program.cpp options.cpp cxx_proto.pb.cpp
...
HEADERS=type.h variable.h ir_node.h expr.h stmt.h gen_policy.h sym_table.h program.h options.h  cxx_proto.pb.h
HEADERS_SRC=$(addprefix src/, $(HEADERS))
PROTOBUF_DESCR=cxx_proto.proto
PROTOBUF_SRC=$(addprefix src/, $(PROTOBUF_DESCR))
EXECUTABLE=yarpgen

default: $(EXECUTABLE)

$(EXECUTABLE): dir $(SOURCES_SRC) $(HEADERS_SRC) $(OBJS) libyarpgen
	$(CXX) $(OPT) $(LDFLAGS) -o $@ $(OBJS) /usr/local/lib/libprotobuf.so -lpthread
...
src/cxx_proto.pb.cpp: $(PROTOBUF_SRC)
	protoc $(PROTOBUF_SRC) --cpp_out=src
	mv src/cxx_proto.pb.cc src/cxx_proto.pb.cpp

src/cxx_proto.pb.h: $(PROTOBUF_SRC)
	protoc --proto_path=$(PROTOBUF_SRC) --cpp_out=src
